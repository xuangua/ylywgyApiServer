...
// GetMgoSession 获取新的session，使用前先调用函数InitializeMongodb初始化session，否则抛出异常
func GetMgoSession() Sessioner {
	if session == nil {
...
func InitializeMongodb(url string, db string) error {
	var err error
	fmt.Println(url)
	fmt.Println(db)
...
func (d *DefaultSession) FindOne(collection string, result interface{}, selector bson.M, fields bson.M) error {
	err := d.newSession.DB(dbName).C(collection).Find(selector).Select(fields).One(result)
	if err != nil {
		if d.printLog {
			d.printLog = false
...
func (d *DefaultSession) FindAll(collection string, result interface{}, selector bson.M, fields bson.M, skip int, limit int, sort ...string) error {
	err := d.newSession.DB(dbName).C(collection).Find(selector).Select(fields).Sort(sort...).Skip(skip * limit).Limit(limit).All(result)
	if err != nil {
		if d.printLog {
			d.printLog = false
			logger.WithFields(
...
func (d *DefaultSession) UpdateOne(collection string, selector bson.M, update bson.M) error {
	...
	err = d.newSession.DB(dbName).C(collection).Update(selector, update)
	if err != nil {
		if d.printLog {
			d.printLog = false
			if err == ErrNotFound {
...
func (d *DefaultSession) UpdateAll(collection string, selector bson.M, update bson.M) (int, error) {
	...
	changeInfo, err := d.newSession.DB(dbName).C(collection).UpdateAll(selector, update)
	if err != nil {
		if d.printLog {
			d.printLog = false
			logger.WithFields(
...
func (d *DefaultSession) DeleteOne(collection string, selector bson.M) error {
	err := d.newSession.DB(dbName).C(collection).Update(selector, bson.M{})
	if err != nil {
		if d.printLog {
			d.printLog = false
			if err == ErrNotFound {
...
func (d *DefaultSession) DeleteAll(collection string, selector bson.M) (int, error) {
	changeInfo, err := d.newSession.DB(dbName).C(collection).UpdateAll(selector, bson.M{})
	if err != nil {
		if d.printLog {
			d.printLog = false
			logger.WithFields(
...
func (d *DefaultSession) FindAndModify(collection string, result interface{}, selector bson.M, update bson.M) error {
	...
	changeInfo, err := d.newSession.DB(dbName).C(collection).Find(selector).Apply(change, result)
	if err != nil {
		if d.printLog {
			d.printLog = false
			if err == ErrNotFound {
