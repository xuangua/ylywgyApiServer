...
export default {
  ...
  async CREATE_ARTICLE ({ commit, state, getters }, params) {
    const { data } = await axios.post(`${getters.baseUrl}/articles/create`, JSON.stringify(params))
    return data
  },
  ...
  async UPDATE_ARTICLE ({ commit, state, getters }, params) {
    const { data } = await axios.patch(`${getters.baseUrl}/articles/update`, params)
    return data
  },
  ...
  async LOGIN ({ commit, state, getters }, user) {
    const { data } = await axios.post(`${getters.routerBaseApi}/signin`, user)
  ...
  async LOGOUT ({ commit, state, getters }) {
    const { data } = await axios.post(`${getters.baseUrl}/signout`)
    return data
  }
}
