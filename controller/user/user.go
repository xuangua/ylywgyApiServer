...
import (
    ...
    "github.com/xuangua/ylywgyApiServer/controller/mail"
...
func Signup(c *gin.Context) {
    ...
    //     }
    // }
    if err := model.DB.Where("name = ?", userData.Name).Find(&user).Error; err == nil {
        if user.Name == userData.Name {
            SendErrJSON("用户名 "+user.Name+" 已被注册", c)
            return
        }
    ...
    // TODO: hack active it here.
    updatedData := map[string]interface{}{
        "status":       model.UserStatusActived,
        "activated_at": time.Now(),
    }

    if err := model.DB.Model(&newUser).Updates(updatedData).Error; err != nil {
        SendErrJSON("error", c)
        return
    }
...
// 获取用户 收件地址列表
func GetCustomerShipAddrList(c *gin.Context) {
    SendErrJSON := common.SendErrJSON
    ...
    // var user_id int
    // var idErr error
    // if user_id, idErr = strconv.Atoi(c.Param("user_id")); idErr != nil {
    //     SendErrJSON("无效的 user_id", c)
    //     return
    // }
    ...
    // 获取 this customer user shipAddressList, if not exist, send error msg.
    var userShipAddr model.CustomerShipAddr
    err := mgoConn.FindOne("customerShipAddr", &userShipAddr, bson.M{"user_id": user_id}, nil)
    if err == mgo.ErrNotFound {
        c.JSON(http.StatusOK, gin.H{
            "errNo": model.ErrorCode.NotShipAddr,
            "type":    "SHIP_ADDRESS_NOT_EXISTS",
            "message": "未设置收货地址",
        })
        return
    } else if err != nil {
        fmt.Println(err.Error())
        SendErrJSON("查找用户 收货地址 错误", c)
        return
    }
    ...
    c.JSON(http.StatusOK, gin.H{
        ...
        "msg":   "success",
        "data": gin.H{
            "userShipAddrArray": userShipAddr.ShipAddrArray,
        },
    })
}
...
// 新增用户 订单配送地址
func AddCustomerShipAddr(c *gin.Context) {
    SendErrJSON := common.SendErrJSON
    ...
    var shipAddr model.ShipAddr
    if err := c.ShouldBindWith(&shipAddr, binding.JSON); err != nil {
        SendErrJSON("订单配置地址 无效", c)
        return
    }
    // shipAddr.
    ...
    // verify if this user have ShipAddressList exist.
    // if exist: update, if not exist: add new one.
    var userShipAddr model.CustomerShipAddr
    err := mgoConn.FindOne("customerShipAddr", &userShipAddr, bson.M{"user_id": user_id}, nil)
    if err == mgo.ErrNotFound {
        shipAddr.Id = 1
        userShipAddr.UserId = user_id
        userShipAddr.ShipAddrArray = append(userShipAddr.ShipAddrArray, shipAddr)
        if err = mgoConn.Insert("customerShipAddr", &userShipAddr); err != nil {
            fmt.Println(err)
            SendErrJSON("创建 userShipAddr 数据库失败.", c)
            return
        }
    } else if err != nil {
        fmt.Println(err.Error())
    ...
    shipAddr.Id = len(userShipAddr.ShipAddrArray) + 1
    userShipAddr.ShipAddrArray = append(userShipAddr.ShipAddrArray, shipAddr)
    ...
    update := bson.M{"$set": bson.M{
        "ship_addr_array":    userShipAddr.ShipAddrArray}}
    err = mgoConn.UpdateOne("customerShipAddr", selector, update)
    // if err != nil && err != mgo.ErrNotFound {
    if err != nil {
        fmt.Println(err)
        SendErrJSON("更新 userShipAddr 数据库失败.", c)
    }
