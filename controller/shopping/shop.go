...
func AddShop(c *gin.Context) {
    ...
    err := mgoConn.FindOne("shops", &shopInfo, bson.M{"name": addShopInfo.Name}, nil)
    if err == nil {
        fmt.Println(err)
    ...
    //  return
    // }
    if err := mgoConn.Insert("shops", &shopInfo); err != nil {
        fmt.Println(err)
        SendErrJSON("创建店铺数据库失败.", c)
        return
    }
...
func UpdateShop(c *gin.Context) {
    ...
    err := mgoConn.UpdateOne("shops", selector, update)
...
func DeleteShop(c *gin.Context) {
    ...
    err = mgoConn.DeleteOneReal("shops", selector)
    if err != nil && err != mgo.ErrNotFound {
        fmt.Println(err)
...
func GetCustomerShopList(c *gin.Context) {
    ...
    var orderBy int
    if orderBy, err = strconv.Atoi(c.Query("order_by")); err != nil {
        SendErrJSON("参数 order_by 无效", c)
        return
    }
    fmt.Printf("orderBy: %v\n", orderBy)

    var schoolName string
    if schoolName = c.Query("school_name"); schoolName == "" {
        SendErrJSON("参数 school_name 无效", c)
        return
    }

    var schoolCampusName string
    if schoolCampusName = c.Query("school_campus_name"); schoolCampusName == "" {
        SendErrJSON("参数 school_campus_name 无效", c)
        return
    }

    var schoolDormName string
    schoolDormName = c.Query("school_dorm_name")
    ...
    if "" == schoolDormName {
        selector = bson.M{
            ...
            "school_campus_name": schoolCampusName}
    } else {
        selector = bson.M{
            "school_name":        schoolName,
            "school_campus_name": schoolCampusName,
            "school_dorm_name":   schoolDormName}            
    }

    sortBy := []string{"-rating", "-recent_order_num"}

    err = mgoConn.FindAll("shops", &shopList, selector, nil, offset, limit, sortBy...)
    if err != nil {
        fmt.Println(err)
        SendErrJSON("error", c)
        return
...
// #### 请求URL：
// ```
// https://xxx/customer/shopping/getShopMenuFoods
// ```
// #### 示例：
// [https://elm.cangdu.org/customer/shopping/getShopMenuFoods?shop_id=8]
// #### 请求方式：
// ```
// GET
// ```
...
// |参数|是否必选|类型|说明|
// |:-----|:-------:|:-----|:-----|
// |shop_id      |Y       |string  |餐馆id|

// GetCustomerShopMenuFoodsList 获取顾客 所选店铺 菜单及商品 列表
func GetCustomerShopMenuFoodsList(c *gin.Context) {
    SendErrJSON := common.SendErrJSON
    var err error

    var shopId int
    if shopId, err = strconv.Atoi(c.Query("shop_id")); err != nil {
        SendErrJSON("参数 shop_id 无效", c)
        return
    }
    // fmt.Println(shopId)
    ...
    selector := bson.M{"id": shopId}
    var shopInfo model.Shop
    err = mgoConn.FindOne("shops", &shopInfo, selector, nil)
    if err != nil {
        fmt.Println(err)
        SendErrJSON("获取店铺信息失败", c)
        return
    }
    fmt.Printf("shopInfo.Menu.ShopFoodCategory : %v\n", shopInfo.Menu.ShopFoodCategory)

    foodMenuList := make([]model.FoodCategory, len(shopInfo.Menu.ShopFoodCategory))
    copy(foodMenuList, shopInfo.Menu.ShopFoodCategory)
    // fmt.Println(foodMenuList)
    // fmt.Println(shopInfo.Menu.ShopFoodCategory)

    var foodList []model.Foods
    selector = bson.M{"shop_id": shopId}
    err = mgoConn.FindAll("foods", &foodList, selector, nil, 0, 0, "$natural")
    if err != nil {
        fmt.Println(err)
    ...
    for _, food := range foodList {
        // fmt.Printf("food FoodCategoryId : %v\n", food.FoodCategoryId)
        // fmt.Printf("food shop_id: %v\n", food.ShopId)
        for i:=0; i < len(foodMenuList); i++ {
            // fmt.Printf("foodMenuList[i] Id: %v\n", foodMenuList[i].Id)
            if food.FoodCategoryId == foodMenuList[i].Id {
                foodMenuList[i].Foods = append(foodMenuList[i].Foods, food)
            }
        }
    }
    // fmt.Printf("foodMenuList : %v\n", foodMenuList)

    // remote Menu which doesn't have any food
    // create a zero-length slice with the same underlying array
    tmp := foodMenuList[:0]
    for _, menu := range foodMenuList {
        if 0 != len(menu.Foods) {
            tmp = append(tmp, menu)
        }
    }
...
func GetAdminUserShopList(c *gin.Context) {
    ...
    err = mgoConn.FindAll("shops", &shopList, bson.M{"owner_id": user.ID}, nil, offset, limit, "$natural")
    if err != nil {
        fmt.Println(err)
        SendErrJSON("error", c)
        return
...
func GetCustomerTotalShopCount(c *gin.Context) {
    ...
    count, err := mgoConn.CountAll("shops", selector)
    if err != nil {
        fmt.Println(err)
        SendErrJSON("error", c)
        return
