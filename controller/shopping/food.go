...
type AddFoodInfo struct {
    ...
    Activity  []model.FoodActivity `json:"activity"`
}
...
func AddFood(c *gin.Context) {
    ...
    err := mgoConn.FindOne("shops", &shopInfo, bson.M{"id": addFoodInfo.ShopId}, nil)
    ...
    foodInfo.Price = addFoodInfo.Price
    ...
    // 新增商品
    if err := mgoConn.Insert("foods", &foodInfo); err != nil {
        fmt.Println(err)
...
func DeleteFood(c *gin.Context) {
    ...
    err = mgoConn.DeleteOne("foods", selector)
    if err != nil && err != mgo.ErrNotFound {
        fmt.Println(err)
...
func GetAdminUserShopFoodList(c *gin.Context) {
    ...
    var shop model.Shop
    err := mgoConn.FindOne("shops", &shop, bson.M{"id": getFoodListParm.shopId}, nil)
    if err != nil {
        fmt.Println(err)
        SendErrJSON("店铺不存在", c)
        return
    ...
    err = mgoConn.FindAll("foods", &foodList, selector, nil, getFoodListParm.offset, getFoodListParm.limit, "$natural")
    if err != nil {
        fmt.Println(err)
    ...
    count, err := mgoConn.CountAll("foods", selector)
    if err != nil {
        fmt.Println(err)
